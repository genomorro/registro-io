...
(1, 'iner', '["ROLE_USER","ROLE_SUPER_ADMIN","ROLE_ADMIN"]', '$2y$13$QC8jjTPtDTApjMAKgpsWcejNXdsGYvFL.iadTBEO9PiIM.i1eol86', 'Coordinación de sistemas');
...
SELECT
    ...
    END,
    datetime('now', '-' || (abs(random()) % 365) || ' days', '+' || (abs(random()) % 365) || ' days', '-' || (abs(random()) % 8) || ' hours', '+' || (abs(random()) % 4) || ' hours', '-' || (abs(random()) % 59) || ' minutes', '+' || (abs(random()) % 59) || ' minutes'),
    CASE abs(random()) % 3
...
SELECT
    ...
    1000 + abs(random()) % 9000,
    CASE abs(random()) % 4
        WHEN 0 THEN 'Radiología' WHEN 1 THEN 'Laboratorio' WHEN 2 THEN 'Consulta Externa' WHEN 3 THEN 'Urgencias' ELSE 'Servicio clínico ' || ((abs(random()) % 10) + 1)
    END,
    datetime('now', '-' || (abs(random()) % 100) || ' hours'),
    CASE WHEN (abs(random()) % 10) > 1 THEN datetime('now', '-' || (abs(random()) % 24) || ' hours') ELSE NULL END,
    CASE abs(random()) % 8

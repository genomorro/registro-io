...
En este momento en los templates 'search/index.html.twig' y 'search/check_out.html.twig' uso conjuntos de tabs de Bootstrap: `<ul class="nav nav-pills nav-fill flex-column flex-sm-row" id="nav-tab" role="tablist">`

Estas listas necesitan un elemento con clase 'active' y 'show'. Estas clases están fijas en los templates actualmente. Quiero que se conserven esas clases en caso de recargar la página, por ejemplo:

En search/index.html.twig:
- Comportamiento actual: Cada tab tiene un formulario de búsqueda. Al entrar en la ruta se selecciona la tab con id nav-01. Si cambio de tab y la búsqueda no fue exitosa, la ruta se recarga, muestra el mensaje correspondiente y el tab nav-01 nuevamente.
- Comportamiento esperado: Cada tab tiene un formulario de búsqueda. Al entrar en la ruta se selecciona la tab con id nav-01. Si cambio de tab y la búsqueda no fue exitosa, la ruta se recarga, muestra el mensaje correspondiente y que el tab seleccionado y mostrado sea aquel donde hice la búsqueda.

Podrías ajustar ese comportamiento.

...
```
Ese servicio en realidad va a ejecutar tres comandos: `php bin:console app:import-data:patient`, `php bin:console app:import-data:appointment`, `php bin:console app:import-data:hospitalized`. En esta ocasión solo vas a crear `php bin:console app:import-data:appointment`, que corresponde al código Python siguiente:
```python
#!/usr/bin/python3
...
```
Es importante la consistencia de los valores de la base de datos, incluyendo que el valor de `idCita` de la tabla origen debe ser el `id` de la tabla destino. Además procura que el valor de `appointment` en la tabla `sqlite_sequence` de la base de datos destino sea igual al valor máximo de `id` de la tabla `appointment`.

***

Información adicional:

1. La base de datos origen tiene una tabla llamada `citasmedicas` con las siguientes columnas: ['idCita', 'idPac', 'fechaCita', 'nombreUsr', 'primerApellidoUsr', 'segundoApellidoUsr', 'fechaHoraElaboracion', 'estatusCita', 'lugRealizacion', 'ubicacion', 'especialidad', 'tipoConsulta']

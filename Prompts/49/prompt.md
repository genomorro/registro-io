...
Revisa RegistrationFormType.php, el comportamiento extraño que observo es el siguiente:

Al agregar un usuario desde la ruta `app_register` de forma predeterminada parece que se selecciona el valor "ROLE_USER" para el campo 'roles', el cual además es obligatorio. Sin embargo, si no se cambia ese campo en el formulario, se puede guardar un registro que aparecerá en la base de datos con el campo 'roles' vacío.

Que comportamiento espero:

1. Al entrar al formulario, el campo 'roles' ya tiene el valor "ROLE_USER" seleccionado.
2. Si selecciono 'ROLE_SUPER_ADMIN' al enviar el formulario, en la base de datos debe guardarse `["ROLE_USER","ROLE_ADMIN","ROLE_SUPER_ADMIN"]`
3. Si selecciono 'ROLE_ADMIN' en el formulario, en la base de datos debe guardarse `["ROLE_USER","ROLE_ADMIN"]`
4. Si selecciono 'ROLE_USER' en el formulario, en la base de datos debe guardarse `["ROLE_USER"]`

Con este comportamiento, será inecesario que el campo 'roles' admita `'multiple' => true,`, debido a que serían seleccionados en automático.

Asegurate que este comportamiento también funcione al editar un registro

...
En el controlador PatientController existen tres métodos con tres rutas:

1. checkOut
2. CheckOutShow
3. checkOutTag

Esas funciones son idénticas, solo se diferencian que la línea `return $this->redirectToRoute()` lleva a tres rutas distintas. Requiero que unifiques esas funciones para que solo exista `checkOut` con la ruta `app_patient_check_out` y reciba como argumento a donde va a redirigir.

Posteriormente, busca el uso de las tres funciones entre todos los templates, y reescribe el código para que use esta nueva función.

...
Acabo de incorporar un nuevo controlador llamado Scheduled. Tiene el CRUD básico ya elaborado, lo que necesito es validar lo siguiente:

   - Si un Area esta siendo usada en la entidad Scheduled (propiedad area_id), el Area no puede ser eliminada de la base de datos.
   - begin_at y end_at son fechas, end_at puede ser un valor igual o mayor a begin_at.
   - El campo "label" debe ser único, por lo que no puede haber dos label iguales.

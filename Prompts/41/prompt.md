...
Se acaba de crear una nueva entidad, Stakeholder. Existe su controlador, su formulario y sus templates.

Quiero que se hagan los siguientes cambios:

1. En el formulario de `app_stakeholder_new` debe aparecer prellenado con la fecha y hora actual el campo checkInAt. Este funcionamiento es como el que muestra la entidad Visitor.
2. En el formulario de `app_stakeholder_new` no debe aparecer el campo checkOutAt, debido a que no es necesario, sin embargo, en `app_stakeholder_edit` debe aparecer y ser prellenado con la fecha y hora actual. Este funcionamiento es como el que muestra la entidad Visitor.
3. En la tabla de la ruta `app_stakeholder_index`, está la columna Check Out, en ella debe aparecer el valor de checkOutAt si es que existe. En caso de que checkOutAt sea un valor vacío, aparecer un botón formulario que registre checkOutAt a la fecha y hora actual. Para ello debe existir un nuevo método en el controlador StakeholderController llamado checkOut y con ruta     `#[Route('/{id}/check-out', name: 'app_stakeholder_check_out', methods: ['POST'])]`. Este comportamiento es como el que se presenta las entidades Visitor y Attendance.

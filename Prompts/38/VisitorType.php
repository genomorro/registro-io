...
class VisitorType extends AbstractType
{
    public function buildForm(FormBuilderInterface $builder, array $options): void
    {
        ...
        $builder->addEventListener(FormEvents::PRE_SET_DATA, function (FormEvent $event) {
            ...
            // DNI logic
            $dniValue = $visitor->getDni();
            if ($dniValue) {
                $dniChoices = $form->get('dni')->getConfig()->getOption('choices');
                if (in_array($dniValue, $dniChoices, true)) {
                    $form->get('dni')->setData($dniValue);
                } else {
                    $form->get('dni')->setData('Otro');
                    $form->get('dni_other')->setData($dniValue);
                }
            }
        });
        ...
        $builder->addEventListener(FormEvents::SUBMIT, function (FormEvent $event) {
            ...
            $dniChoice = $form->get('dni')->getData();
            $dniOtherValue = $form->get('dni_other')->getData();

            if ($dniChoice === 'Otro') {
                $visitor->setDni($dniOtherValue);
            } else {
                $visitor->setDni($dniChoice);
            }
        });
    }

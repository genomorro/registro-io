# KnpPaginatorBundle index tables
...
En el CRUD de la entidad Hospitalized está la ruta `app_hospitalized_index`, donde se muestra una tabla que usa el bundle KnpPaginatorBundle para paginar la tabla que aparece en ese template. Además, esa tabla puede ordenar las columnas y filtrar los resultados.

Quiero que implementes el uso de KnpPaginatorBundle en las siguientes rutas con las siguientes consideraciones:

1. `app_patient_index`: Esta tabla debe filtrarse por file y por name
2. `app_appointment_index`: Esta tabla filtra por Patient name, Appointment agenda y Appointment dateAt
3. `app_attendance_index`: Esta tabla filtra por Patient name y tag
4. `app_visitor_index`: Esta tabla filtra por name y tag
5. `app_stakeholder_index`: Esta tabla filtra por name y tag

***

En la ruta `app_patient_show` se usa PagerFantaBundle, quiero que las tres tablas que actualmente lo usan, dejen de ser paginadas. Ajusta las respectivas consultas DQL, el controlador y el template, para que no se use paginación.

Después, comprueba si ya puedo eliminar PagerFantaBundle del proyecto.

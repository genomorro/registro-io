...
La entidad Visitor y Attendance tienen una propiedad llamada `evidence`, que debe guardar la ruta de una imagen. La imagen se toma al crear un nuevo registro de estas entidades, desde las rutas `app_visitor_new`, `app_attendance_new` y `app_patien_check_in`. Cuando hay una cámara web conectada, la aplicación funciona perfectamente, al momento de crear el registro, captura automáticamente la imagen por medio del controlador de Stimulus `webrtc_controller.js`.

El problema es que en estos momentos, no puede crear un nuevo registro sin la cámara web activa, pese a que la propiedad `evidence` puede quedar vacía en la base de datos. Podrías revisar ese funcionamiento.

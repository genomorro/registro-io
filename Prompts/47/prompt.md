...
En el CRUD de la entidad Hospitalized está la ruta `app_hospitalized_index`, donde se muestra una tabla. Usa el bundle KnpPaginatorBundle para paginar la tabla que aparece en ese template. Muéstrame una imagen de la vista una vez aplicado ese cambio.

*** 

Bien hecho, solo me gustaría que hicieras algunos ajustes:

1. En la configuración de knp_paginator ajustar:
```knp_paginator.yaml
pagination: '@KnpPaginator/Pagination/bootstrap_v5_pagination.html.twig'
```

2. Hacer que las columnas sean ordenables.

***

Me gustaría que también las columnas File y Patient pudieran ordenarse. Revisa que tan viable es esa característica sin comprometer el rendimiento del sitio.

***

Gracias. Finalmente, será posible implementar un filtro, que filtre por File o Name. Si no es posible que sean ambos, que solo sea por File.
